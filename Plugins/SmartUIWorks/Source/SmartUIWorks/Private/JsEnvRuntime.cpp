...
void FJsEnvRuntime::RestartJsScripts(const FString& ScriptHomeDir, const TArray<TPair<FString, UObject*>>& Arguments)
{
	for (auto& Pair : JsRuntimeEnvPool)
	{
		auto Env = Pair.Key;
		Env->Release();
		Env->ForceReloadJsFile(ScriptHomeDir);
		Env->Start(ScriptHomeDir, Arguments);
	}
	...
	for (FString& SourcePath : FileNames)
	{
		...
		RelativePath.RemoveFromEnd(TEXT(".map"));
	...
	for (const FString& ModuleName : ModuleNames)
	{
		...
		{
			auto Env = Pair.Key;
			Env->ForceReloadJsFile(ModuleName);
			Env->Start(ModuleName, Arguments);
		}
	}*/
}