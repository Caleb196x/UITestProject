...
function parseTransform(transform) {
    ...
    const parseAngle = (angle) => {
        ...
        if (!match)
            return 0;
        const value = (0, common_utils_1.safeParseFloat)(match[1]);
    ...
    transformParts.forEach(part => {
        ...
        switch (property) {
            ...
            case 'scale':
            case 'scale3d':
                result.scale.X = (0, common_utils_1.safeParseFloat)(values[0]);
                result.scale.Y = (0, common_utils_1.safeParseFloat)(values[1] || values[0]);
                break;
            case 'scaleX':
                result.scale.X = (0, common_utils_1.safeParseFloat)(values[0]);
                break;
            case 'scaleY':
                result.scale.Y = (0, common_utils_1.safeParseFloat)(values[0]);
                break;
            case 'rotate':
            case 'rotateZ':
            ...
            case 'matrix':
                ...
                // [b d ty]
                // [0 0 1 ]
                result.scale.X = (0, common_utils_1.safeParseFloat)(values[0]); // a - scale x
                result.scale.Y = (0, common_utils_1.safeParseFloat)(values[3]); // d - scale y
                // Convert skew matrix values to angles in degrees (-90 to 90)
                result.shear.X = Math.max(-90, Math.min(90, (0, common_utils_1.safeParseFloat)(values[2])) * 180 / Math.PI); // c - skew x angle
                result.shear.Y = Math.max(-90, Math.min(90, (0, common_utils_1.safeParseFloat)(values[1])) * 180 / Math.PI); // b - skew y angle
                result.translation.X = (0, common_utils_1.safeParseFloat)(values[4]); // tx - translate x
                result.translation.Y = (0, common_utils_1.safeParseFloat)(values[5]); // ty - translate y
                break;
            case 'matrix3d':
                // matrix3d(a, b, 0, 0, c, d, 0, 0, 0, 0, 1, 0, tx, ty, 0, 1)
                // Only use the 2D transformation components
                result.scale.X = (0, common_utils_1.safeParseFloat)(values[0]); // a - scale x
                result.scale.Y = (0, common_utils_1.safeParseFloat)(values[5]); // d - scale y
                result.shear.X = Math.max(-90, Math.min(90, (0, common_utils_1.safeParseFloat)(values[4])) * 180 / Math.PI); // c - skew x angle
                result.shear.Y = Math.max(-90, Math.min(90, (0, common_utils_1.safeParseFloat)(values[1])) * 180 / Math.PI); // b - skew y angle
                result.translation.X = (0, common_utils_1.safeParseFloat)(values[12]); // tx - translate x
                result.translation.Y = (0, common_utils_1.safeParseFloat)(values[13]); // ty - translate y
                break;
        }
    });
    return {
...
function convertCommonPropsToWidgetProps(props) {
    ...
    const result = {};
    {
        ...
        if (opacity) {
            result['RenderOpacity'] = (0, common_utils_1.safeParseFloat)(opacity);
        }
