...
static PyObject* ue_PyUObject_getattro(ue_PyUObject* self, PyObject* attr_name)
{
	...
	if (!ret)
	{
		if (PyUnicodeOrString_Check(attr_name))
		{
			...
			FProperty* u_property = u_struct->FindPropertyByName(FName(UTF8_TO_TCHAR(attr)));
			if (u_property)
			{
	...
	if (self->ue_object->IsA<UScriptStruct>())
	{
		...
		if (kw)
		{
			...
			for (;;)
			{
				...
				FProperty* u_property = ue_struct_get_field_from_name(u_script_struct, (char*)struct_key);
				if (u_property)
				{
...
void unreal_engine_init_py_module()
{
	...
	// PyDict_SetItemString(unreal_engine_dict, "RF_DYNAMIC", PyLong_FromUnsignedLongLong((uint64)RF_Dyn));
...
ue_PyUObject* ue_get_python_uobject(UObject* ue_obj)
{
	...
	if (!ret)
	{
		if (!ue_obj->IsValidLowLevel() /*|| ue_obj->IsGarbageEliminationEnabled()*/)
			return nullptr;
...
PyObject* ue_py_convert_property(FProperty* prop, uint8* buffer, int32 index)
{
	...
	if (auto casted_prop = Cast<FUInt64Property>(prop))
	{
	...
	if (auto casted_prop = Cast<FObjectProperty>(prop))
	{
...
bool ue_py_convert_pyobject(PyObject* py_obj, FProperty* prop, uint8* buffer, int32 index)
{
	...
	if (PyBytes_Check(py_obj))
	{
		if (auto casted_prop = Cast<FArrayProperty>(prop))
		{
			...
			if (auto item_casted_prop = Cast<FArrayProperty>(casted_prop->Inner))
			{
	...
	if (PyObject_IsInstance(py_obj, (PyObject*)& ue_PyUObjectType))
	{
		...
		if (ue_obj->ue_object->IsA<UClass>())
		{
			...
			{
				// casted_prop_soft_class->SetPropertyValue_InContainer(buffer, *ue_obj->ue_object, index);
				return true;
			}
			else if (auto casted_prop_soft_object = Cast<FSoftClassProperty>(prop))
			{
...
{
	FProperty* u_property = u_obj->ue_object->GetClass()->FindPropertyByName(FName(*event_name));
	if (!u_property)
	{
...
/*#if ENGINE_MINOR_VERSION < 18
	...
	TObjectPtr<UField>* next_property = &function->Children;
	...
	while (annotations)
	{
		...
		if (PyType_Check(value))
		{
			...
			else if ((PyTypeObject*)value == &ue_PyFVectorType)
			{
				...
				prop = new FStructProperty(function, UTF8_TO_TCHAR(p_name), RF_Public);
			}
		...
		if (prop)
		{
			...
			// *next_property = prop;
			/*next_property = TObjectPtr<UField>(prop->Next);
		...
		if (py_return_value)
		{
			...
			if (PyType_Check(py_return_value))
			{
				...
				else if (PyObject_IsInstance(py_return_value, (PyObject*)& PyType_Type))
				{
					...
					// prop_class->SetMetaClass((UClass*)py_obj->ue_object);
					// prop_class->PropertyClass = UClass::StaticClass();
					prop = new FClassProperty(function, UTF8_TO_TCHAR(p_name), RF_Public);
					Py_DECREF(type_args);
				}
			}
			else if (ue_PyUObject * py_obj = ue_is_pyuobject(py_return_value))
			{
				if (py_obj->ue_object->IsA<UClass>())
				{
					...
					// UObjectProperty* prop_base = NewObject<UObjectProperty>(function, UTF8_TO_TCHAR(p_name), RF_Public);
					// prop_base->SetPropertyClass(p_u_class);
					prop = new FObjectProperty(function, UTF8_TO_TCHAR(p_name), RF_Public);
				}
#if ENGINE_MAJOR_VERSION > 4
				else if (py_obj->ue_object->IsA<UEnum>())
				{
					...
					// prop_enum->SetEnum((UEnum*)py_obj->ue_object);
					// prop_enum->AddCppProperty(prop_underlying);
					prop = new FEnumProperty(function, UTF8_TO_TCHAR(p_name), RF_Public);
				}
#endif
				else if (py_obj->ue_object->IsA<UStruct>())
				{
		...
		if (!function->IsSignatureCompatibleWith(parent_function))
		{
			...
			while (It)
			{
				...
				if (p->PropertyFlags & CPF_Parm)
				{
					...
					if (ucp)
					{
						UE_LOG(LogPython, Warning, TEXT("Parent UClassProperty = %p %s %p %s"), ucp->PropertyClass.Get(), *ucp->PropertyClass->GetName(), ucp->MetaClass.Get(), *ucp->MetaClass->GetName());
					}*/
				}
				++It;
			}
			...
			while (It2)
			{
				...
				if (p->PropertyFlags & CPF_Parm)
				{
					...
					if (ucp)
					{
						UE_LOG(LogPython, Warning, TEXT("Function UClassProperty = %p %s %p %s"), ucp->PropertyClass.Get(), *ucp->PropertyClass->GetName(), ucp->MetaClass.Get(), *ucp->MetaClass->GetName());
					}*/
				}
				++It2;
			}
