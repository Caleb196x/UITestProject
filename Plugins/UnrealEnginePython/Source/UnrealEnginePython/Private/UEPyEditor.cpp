...
PyObject *py_unreal_engine_create_blueprint_from_actor(PyObject * self, PyObject * args)
{
	...
	UBlueprint *bp = FKismetEditorUtilities::CreateBlueprintFromActor(UTF8_TO_TCHAR(name), actor);
...
PyObject *py_unreal_engine_add_level_to_world(PyObject *self, PyObject * args)
{
	...
	UClass *streaming_mode_class = ULevelStreaming::StaticClass();
	if (py_bool && PyObject_IsTrue(py_bool))
	{
...
PyObject *py_unreal_engine_heightmap_expand(PyObject * self, PyObject * args)
{
	...
	TArray<uint16> data;
	FIntRect ResourceRegion(0, 0, width - 1, height - 1);
	FIntRect DestRegion(-offset_x, -offset_y, new_width - offset_x - 1, new_height - offset_y - 1);
	FLandscapeConfigHelper::ExpandData<uint16>(original_data, data, ResourceRegion, DestRegion, true);
