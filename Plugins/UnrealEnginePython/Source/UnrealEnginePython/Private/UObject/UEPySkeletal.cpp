...
/*#if ENGINE_MINOR_VERSION > 20
	...
	while (PyObject *py_item = PyIter_Next(py_iter))
	{
		...
		FVector pos = static_cast<FVector>(ss_vertex->ss_vertex.Position);
		int32 vertex_index = points.Add(pos);
...
/*#if ENGINE_MINOR_VERSION > 20
		...
		tangentsX.Add(static_cast<FVector>(ss_vertex->ss_vertex.TangentX));
		tangentsY.Add(static_cast<FVector>(ss_vertex->ss_vertex.TangentY));
		tangentsZ.Add(static_cast<FVector>(ss_vertex->ss_vertex.TangentZ));
...
/*#if ENGINE_MINOR_VERSION > 20
		...
		face.TangentX[0] = static_cast<FVector3f>(tangentsX[i]);
		face.TangentX[1] = static_cast<FVector3f>(tangentsX[i + 1]);
		face.TangentX[2] = static_cast<FVector3f>(tangentsX[i + 2]);

		face.TangentY[0] = static_cast<FVector3f>(tangentsY[i]);
		face.TangentY[1] = static_cast<FVector3f>(tangentsY[i + 1]);
		face.TangentY[2] = static_cast<FVector3f>(tangentsY[i + 2]);

		face.TangentZ[0] = static_cast<FVector3f>(tangentsZ[i]);
		face.TangentZ[1] = static_cast<FVector3f>(tangentsZ[i + 1]);
		face.TangentZ[2] = static_cast<FVector3f>(tangentsZ[i + 2]);
...
/*#if ENGINE_MINOR_VERSION < 19
	...
	TArray<FVector3f> Points3f;
	Points3f.Reserve(points.Num());
	for (int32 i = 0; i < points.Num(); ++i)
	{
		Points3f[i] = static_cast<FVector3f>(points[i]);
	}
	bool success = MeshUtilities.BuildSkeletalMesh(lod_model, "", mesh->RefSkeleton, influences, wedges, faces, Points3f, points_to_map, build_settings);
