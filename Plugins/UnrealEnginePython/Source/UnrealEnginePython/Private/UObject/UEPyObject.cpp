...
{
	if (!self->ue_object || !self->ue_object->IsValidLowLevel() || self->ue_object->IsPendingKillEnabled())
	{
		Py_RETURN_FALSE;
	}
...
PyObject *py_ue_get_uproperty(ue_PyUObject *self, PyObject * args)
{
	...
	Py_RETURN_UOBJECT(u_property->GetOwnerUObject());
...
PyObject *py_ue_add_property(ue_PyUObject * self, PyObject * args)
{
	...
	// u_property = New<FProperty>();
	if (!u_property)
	{
		if (is_array || is_map)
			scope->OnClusterMarkedAsPendingKill();
		return PyErr_Format(PyExc_Exception, "unable to allocate new UProperty");
	}
