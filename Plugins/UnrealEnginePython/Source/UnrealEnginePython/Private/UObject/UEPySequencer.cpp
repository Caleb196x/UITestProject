...
#endif
	CurveAPI.GetConvertedTransformCurveData(NodeName, Translation[0], Translation[1], Translation[2],
		EulerRotation[0], EulerRotation[1], EulerRotation[2], Scale[0],
		Scale[1], Scale[2], DefaultTransform, false);
...
PyObject *py_ue_sequencer_changed(ue_PyUObject *self, PyObject * args)
{
	...
	if (py_bool && PyObject_IsTrue(py_bool))
	{
		...
		AssetViewUtils::OpenEditorForAsset(seq);
	}

	IAssetEditorInstance *editor = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(seq, /*bFocusIfOpen*/true);
	if (editor)
	{
...
#if ENGINE_MAJOR_VERSION > 4
	TRange<FFrameNumber> FrameNum;
	section->SetRange(FrameNum);
#else
...
PyObject *py_ue_sequencer_import_fbx_transform(ue_PyUObject *self, PyObject * args)
{
	...
	for (FString NodeName : AllNodeNames)
	{
		...
		FTransform DefaultTransform;
#if ENGINE_MAJOR_VERSION >= 4
		CurveAPI.GetConvertedTransformCurveData(NodeName, Translation[0],
			Translation[1], Translation[2], EulerRotation[0],
			EulerRotation[1], EulerRotation[2], Scale[0], Scale[1], Scale[2], DefaultTransform, false);
/*#if ENGINE_MINOR_VERSION < 20
