...
#endif*/
	// auto info = self->foliage_actor->GetFoliageInfos()[self->foliage_type.Get()];

	/*if (self->instance_id >= 0 && self->instance_id < info.Instances.Num())
	{
		return info.Get().Instances[self->instance_id];
	}*/
...
/*#if ENGINE_MINOR_VERSION >= 23
			...
			FFoliageMeshInfo& info = self->foliage_actor->FoliageMeshes[self->foliage_type.Get()].Get();
#endif*/
			/*auto& info = self->foliage_actor->GetFoliageInfos()[self->foliage_type.Get()].Get();
			info.PreMoveInstances(self->foliage_actor.Get(), instances);
...
{
	get_instance(self);
	return py_ue_new_fvector(static_cast<FVector>(instance->DrawScale3D));
}
