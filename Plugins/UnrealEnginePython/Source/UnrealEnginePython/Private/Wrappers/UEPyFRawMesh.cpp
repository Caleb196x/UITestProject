...
static PyObject *py_ue_fraw_mesh_set_vertex_positions(ue_PyFRawMesh *self, PyObject * args)
{
	...
	self->raw_mesh.VertexPositions.Reserve(vertex.Num());
	for(int i = 0; i < vertex.Num(); ++i)
	{
		self->raw_mesh.VertexPositions[i] = static_cast<FVector3f>(vertex[i]);
	}
...
static PyObject *py_ue_fraw_mesh_set_wedge_tex_coords(ue_PyFRawMesh *self, PyObject * args)
{
	...
	/*self->raw_mesh.VertexPositions.Reserve(uv.Num());
	for(int i = 0; i < uv.Num(); ++i)
	{
		self->raw_mesh.WedgeTexCoords[i] = static_cast<FVector2f>(uv[i]);
	}*/
...
static PyObject *py_ue_fraw_mesh_get_wedge_position(ue_PyFRawMesh *self, PyObject * args)
{
	...
	FVector3f vec = self->raw_mesh.GetWedgePosition(index);

	return py_ue_new_fvector(static_cast<FVector>(vec));
}
...
static PyObject *py_ue_fraw_mesh_set_wedge_tangent_x(ue_PyFRawMesh *self, PyObject * args)
{
	...
	for (int i = 0; i < vertex.Num(); ++i)
	{
		self->raw_mesh.WedgeTangentX[i] = static_cast<FVector3f>(vertex[i]);
	}
...
static PyObject *py_ue_fraw_mesh_set_wedge_tangent_y(ue_PyFRawMesh *self, PyObject * args)
{
	...
	for (int i = 0; i < vertex.Num(); ++i)
	{
		self->raw_mesh.WedgeTangentY[i] = static_cast<FVector3f>(vertex[i]);
	}
...
static PyObject *py_ue_fraw_mesh_set_wedge_tangent_z(ue_PyFRawMesh *self, PyObject * args)
{
	...
	for (int i = 0; i < vertex.Num(); ++i)
	{
		self->raw_mesh.WedgeTangentZ[i] = static_cast<FVector3f>(vertex[i]);
	}
...
static PyObject *py_ue_fraw_mesh_get_vertex_positions(ue_PyFRawMesh *self, PyObject * args)
{
	...
	{
		PyList_Append(py_list, py_ue_new_fvector(static_cast<FVector>(self->raw_mesh.VertexPositions[i])));
	}
...
static PyObject *py_ue_fraw_mesh_get_wedge_tangent_x(ue_PyFRawMesh *self, PyObject * args)
{
	...
	{
		PyList_Append(py_list, py_ue_new_fvector(static_cast<FVector>(self->raw_mesh.WedgeTangentX[i])));
	}
...
static PyObject *py_ue_fraw_mesh_get_wedge_tangent_y(ue_PyFRawMesh *self, PyObject * args)
{
	...
	{
		PyList_Append(py_list, py_ue_new_fvector(static_cast<FVector>(self->raw_mesh.WedgeTangentY[i])));
	}
...
static PyObject *py_ue_fraw_mesh_get_wedge_tangent_z(ue_PyFRawMesh *self, PyObject * args)
{
	...
	{
		PyList_Append(py_list, py_ue_new_fvector(static_cast<FVector>(self->raw_mesh.WedgeTangentZ[i])));
	}
