...
static PyObject *py_ue_fraw_anim_sequence_track_get_pos_keys(ue_PyFRawAnimSequenceTrack *self, void *closure)
{
	...
	for (FVector3f vec : self->raw_anim_sequence_track.PosKeys)
	{
		PyObject *py_vec = py_ue_new_fvector(static_cast<FVector>(vec));
...
{
	PyObject *py_list = PyList_New(0);
	for (FVector3f vec : self->raw_anim_sequence_track.ScaleKeys)
	{
		PyObject *py_vec = py_ue_new_fvector(static_cast<FVector>(vec));
		PyList_Append(py_list, py_vec);
		Py_DECREF(py_vec);
...
{
	PyObject *py_list = PyList_New(0);
	for (FQuat4f quat : self->raw_anim_sequence_track.RotKeys)
	{
		PyObject *py_quat = py_ue_new_fquat(static_cast<FQuat>(quat));
...
static int py_ue_fraw_anim_sequence_track_set_pos_keys(ue_PyFRawAnimSequenceTrack *self, PyObject *value, void *closure)
{
	...
	if (value)
	{
		...
		if (py_iter)
		{
			...
			if (!failed)
			{
				TArray<FVector3f> pos_3f;
				pos_3f.Reserve(pos.Num());
				for (int i = 0; i < pos.Num(); ++i)
				{
					pos_3f[i] = static_cast<FVector3f>(pos[i]);
				}
				self->raw_anim_sequence_track.PosKeys = pos_3f;
				return 0;
			}
		}
	}
...
static int py_ue_fraw_anim_sequence_track_set_scale_keys(ue_PyFRawAnimSequenceTrack *self, PyObject *value, void *closure)
{
	...
	if (value)
	{
		...
		if (py_iter)
		{
			...
			if (!failed)
			{
				self->raw_anim_sequence_track.ScaleKeys.Reserve(scale.Num());
				for (int i = 0; i < scale.Num(); ++i)
				{
					self->raw_anim_sequence_track.ScaleKeys[i] = static_cast<FVector3f>(scale[i]);
				}
				return 0;
			}
		}
...
static int py_ue_fraw_anim_sequence_track_set_rot_keys(ue_PyFRawAnimSequenceTrack *self, PyObject *value, void *closure)
{
	...
	if (value)
	{
		...
		if (py_iter)
		{
			...
			if (!failed)
			{
				self->raw_anim_sequence_track.RotKeys.Reserve(rot.Num());
				for (int i = 0; i < rot.Num(); ++i)
				{
					self->raw_anim_sequence_track.RotKeys[i] = static_cast<FQuat4f>(rot[i]);
				}
				return 0;
			}
		}
