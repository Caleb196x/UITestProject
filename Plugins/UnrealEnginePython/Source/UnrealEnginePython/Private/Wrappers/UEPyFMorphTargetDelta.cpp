...
{
	return py_ue_new_fvector(static_cast<FVector>(self->morph_target_delta.PositionDelta));
}
...
static int py_ue_fmorph_target_delta_set_position_delta(ue_PyFMorphTargetDelta *self, PyObject *value, void *closure)
{
	...
	if (py_vec)
	{
		self->morph_target_delta.PositionDelta = static_cast<FVector3f>(py_vec->vec);
		return 0;
	}
...
{
	return py_ue_new_fvector(static_cast<FVector>(self->morph_target_delta.TangentZDelta));
}
...
static int py_ue_fmorph_target_delta_set_tangent_z_delta(ue_PyFMorphTargetDelta *self, PyObject *value, void *closure)
{
	...
	if (py_vec)
	{
		self->morph_target_delta.TangentZDelta = static_cast<FVector3f>(py_vec->vec);
		return 0;
	}
	PyErr_SetString(PyExc_TypeError, "value is not a FVector");
	return -1;
