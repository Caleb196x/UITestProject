...
bool UXmlParser::SerializeToXml(UXmlUmgTree* UmgTree)
{
	...
	auto ParseNodeToXmlElement = [&XmlDoc](const UXmlUmgNode* Node) -> tinyxml2::XMLElement*
	{
		...
		bool bHasChildObjectProperty = false;
		for (const auto& Attr : Node->Properties)
		{
			...
			if (Attr.Value.ChildAttributes.Num() > 0)
			{
				bHasChildObjectProperty = true;
			}
		}

		if (bHasChildObjectProperty)
		{
			tinyxml2::XMLElement* XmlPropertiesElement = XmlElement->InsertNewChildElement(ExtraAttributeXmlKeyword);
			for (const auto& Attr : Node->Properties)
			{
				FString PropName = Attr.Key;
